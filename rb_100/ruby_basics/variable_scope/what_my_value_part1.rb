...
to the `my_value` invocation, returning `17`. Arguments are 
...
Methods are self-contained, accessing data from the outer scope only 
through arguments and parameters. 

The `+=` method isn't mutating, and it doesn't mutate object (`7` -
immutable data type) in the outer scope (line 2). 
The `+=` returns a new object. This demonstrates Ruby's 
behavior as if pass-by-value.
