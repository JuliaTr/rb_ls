...
but 98 is not (it is not odd), 97 is not (it is not a multiple 
...
PEDAC
P: return the next number which is divided by 7 without a 
remainder and whose digits occur exactly once each.
=end

## Possible solution:
def featured(number)
  # Find odd number and it's divisible by 7:
  number += 1
  number += 1 until number.odd? && number % 7 == 0
  
  # Define if number's digits don't repeat.
  loop do
