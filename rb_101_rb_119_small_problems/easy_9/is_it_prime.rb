=begin
A prime number is a positive number that is evenly divisible 
only by itself and 1. Thus, 23 is prime since its only 
divisors are 1 and 23. However, 24 is not prime since it has 
divisors of 1, 2, 3, 4, 6, 8, 12, and 24. Note that the number 
1 is not prime.

Write a method that takes a positive integer as an argument 
and returns true if the number is prime, false if it is not 
prime.

You may not use Ruby's Prime class to solve this problem. 
Your task is to programmatically determine whether a number 
is prime without relying on any methods that already do that 
for you.
=end

def array_of_subsequent_integers(number)
  arr = []
...
def is_prime(number)
  ...
  # Create array of all subsequent integers upto `number`.
  arr = array_of_subsequent_integers(number)

  # Find if `number` has divisors other than 1 and `number`.
  index = 1
  current_number = nil
  loop do
    current_number = arr[index]
    break if number % current_number == 0
    index += 1
  end
