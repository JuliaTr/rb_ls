...
removes the value from the stack, and then stores the 
result back in the register. Thus, if we start with a 
stack of `3 6 4` (where `4` is the topmost item in the stack), 
and a register value of `7`, then the `MULT` operation will 
transform things to `3 6` on the stack (the `4` is removed), 
and the result of the multiplication, `28`, is left in the 
register. If we do another `MULT` at this point, then the 
stack is transformed to `3`, and the register is left with 
the value `168`.

Write a method that implements a miniature stack-and-register-based programming language that has the following commands:

- `n` - Place a value n in the "register". Do not modify the stack.
- `PUSH` - Push the register value on to the stack. Leave the 
...
- `MOD` - Pops a value from the stack and divides it into the 
   register value, storing the integer remainder of the division 
   in the register.
...
def minilang(str)
  ...
  str.split.each do |operation|
    register = operation.to_i if operation.to_i.to_s == operation  # check if integer
    ...
    when 'MOD'   then register = register.divmod(stack.pop)[-1]
