=begin
6. Create a method that takes an array of integers 
and returns an array containing all pairs of numbers 
in the original array that sum to an even number. Each 
pair should be sorted in ascending order, and the entire 
array of pairs should be sorted by the first element of 
each pair.
...
def pairs(arr)
  ...
  0.upto(arr.size - 1) do |first|
    first.upto(arr.size - 1) do |second|
      pair = [arr[first], arr[second]]
      result << pair if pair.sum.even?
    end
  end
...
def even_sum_pairs(arr)
  ...
  pairs = pairs(arr)
  pairs.sort.map { |subarray| subarray.sort }
end
