...
loop do
  ...
  # loop do
    ...
    # prompt('zero_error')
    if chosen_operator == '4' && second_number == 0
      prompt('zero_error')
    else
      prompt(operation_message[chosen_operator])

      case chosen_operator
      when '1' then puts first_number + second_number
      when '2' then puts first_number - second_number
      when '3' then puts first_number * second_number
      when '4'
        if first_number % second_number == 0
          puts first_number / second_number
        else
          dividend = first_number.to_f() / second_number.to_f()
          puts dividend
          case dividend
          when 0..4 then puts dividend.floor
          when 5..9 then puts dividend.ceil
          end
        end
      end
    end
