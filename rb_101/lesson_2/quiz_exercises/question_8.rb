...
Iterate through the words one by one.
  ...
  - starting with the next word iterate through all the remaining 
...
p longest_word('I love programming with Ruby')
# "programming"



## Don't match the pseudocode:
def longest_word(sentence)
  words = sentence.split
  saved_word = words[0]
...
=begin
Iteration strts with the first word, but not from the second.
There's no comparison if the is the same length as the first one.
=end
...
def longest_word(sentence)
  ...
  words.each do |word|
    if saved_word.length < word.length  # wrong comparison
      save_word = word
    end
  end
...
p longest_word('I love programming with Ruby')  # I
